...
export function TreasureChest({ progress, latestItem }: Props) {
  ...
  // Lid opening blend: keep hinge visually attached to chest top while opening.
  // We combine gentle squash + slight backward tilt + tiny upward lift.
  const lidScaleY = Math.max(0.42, 1 - fillRatio * 0.58);
  const lidScaleX = 1 - (1 - lidScaleY) * 0.04;
  const lidTilt = -Math.min(11, fillRatio * 11);
  const lidLift = (1 - lidScaleY) * 3.2;
    ...
    <div className="relative flex flex-col items-center">
      ...
      >
        ...
        >
          ...
          {/* ═══════════════════════════════════════════════════════════════
              LAYER 3 — LID  (attached hinge + backward open illusion)
              The animated shell pivots/squashes, while hinge bar stays fixed
              on the chest rim so the lid never looks detached.
          ═══════════════════════════════════════════════════════════════ */}
          <motion.g
            style={{ transformOrigin: '100px 101px' }}
            animate={{ scaleY: lidScaleY, scaleX: lidScaleX, rotate: lidTilt, y: -lidLift }}
